...
void buttonpress(XEvent *e) {
  ...
  if (ev->window == selmon->barwin) {
    ...
    do {
      ...
      // if (!(occ & 1 << i || m->tagset[m->seltags] & 1 << i)) //Commented cus
      // i want all to show regardless its vacant 	continue;
      x += TEXTW(tags[i]);
...
void drawbar(Monitor *m) {
  ...
  /* Added drawing window title functionality */
  // UNCOMMENT THIS TO NOT DRAW TITLE
  // if ((w = m->ww - tw - x) > bh) {
  //   drw_setscheme(drw, scheme[SchemeNorm]);
